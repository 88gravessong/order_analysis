...
def read_orders(file_path: Path):
    ...
    with open(file_path, "rb") as f:
        data = f.read()
    return _iter_rows_stream(BytesIO(data))

def _iter_rows_stream(file_bytes: BytesIO):
    peek = file_bytes.read(4)
    file_bytes.seek(0)
    if peek.startswith(b"PK\x03\x04"):
        ...
        try:
            ...
            file_bytes.seek(0)
            text = file_bytes.read().decode("gbk", errors="ignore")
...
def compute_metrics(file_path: Path):
    ...
    total_rows = 0
    for seller_sku, province, sub, cancel, shipped, _created in read_orders(file_path):
